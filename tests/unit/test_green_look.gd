...
func test_the_green_is_hotter_and_tighter_than_the_fairway() -> void:
	...
	assert_almost_eq(float(green["cell"]), GridSnap.CELL, 0.001, "green shares the extra-small snap cell")
	assert_almost_eq(float(fringe["cell"]), GridSnap.CELL, 0.001, "fringe shares the extra-small snap cell")
	assert_almost_eq(float(fairway["cell"]), GridSnap.CELL, 0.001, "fairway shares the extra-small snap cell")
