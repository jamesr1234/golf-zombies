...
static func armory(root: Node3D) -> void:
	...
	_remote(_bob(root, Vector3(-1.15, 0.48, 0.42), -0.4, 3.6))
...
static func items(root: Node3D) -> void:
	...
	_caption(root, "LADDER", Vector3(2.45, 2.55, 0.78))
	LeanLadder.add_preview(_bob(root, Vector3(2.45, 2.12, 0.42), 0.45, 1.6), 0.16)
	_caption(root, "MECH", Vector3(0.0, 1.15, 0.78))
	MechVisuals.mini(_bob(root, Vector3(0.0, 0.98, 0.42), 0.4, 2.2), 0.10)
...
static func preview(parent: Node3D, item: Dictionary) -> void:
	...
	match String(item.get("id", "")):
		...
		"barrier":
			_hex(parent)
		"ladder":
			LeanLadder.add_preview(parent, 0.28)
		"mech":
		...
		_:
			if not Wear.preview(parent, item):
				return
...
static func _remote(parent: Node3D) -> void:
	parent.rotation.y = deg_to_rad(14.0)
	parent.rotation.z = deg_to_rad(-6.0)
	var shell := Palette.NIGHT
	_box(parent, Vector3(0.22, 0.08, 0.38), shell, Palette.GLOW_FAINT, Vector3.ZERO)
	_cyl(parent, 0.08, 0.07, Palette.LED_RED, Palette.GLOW_STRONG, Vector3(0.0, 0.08, 0.0))
	var stem := _cyl(parent, 0.014, 0.18, shell, 0.0, Vector3(0.0, 0.02, -0.28))
	stem.rotation.x = deg_to_rad(90.0)
	_sphere(parent, 0.03, Palette.LED_RED, Palette.GLOW_MEDIUM, Vector3(0.0, 0.02, -0.4))
	var grip := _box(parent, Vector3(0.1, 0.22, 0.1), shell, 0.0, Vector3(0.0, -0.16, 0.12))
	grip.rotation.x = deg_to_rad(-16.0)
